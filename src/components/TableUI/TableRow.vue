  ...
  >
    <div
      ...
      :class="{
        ...
        [`cell_${rowData.id}_${index}`]: true,
      }"
      :style="{ width: `${columnWidths[column.key]}px` }"
      @click="(e) => handleCellClick(e, column)"
      @hover="(e) => handleCellClick(e, column)"
    >
      <TableCell :column="column" :row-id="rowData.id" :cell-value="rowData[column.key]" />
    </div>
  </div>
</template>
...
const emit = defineEmits<{
  ...
  (e: 'hoverCell', payload: { rowId: string; colKey: string; position: DOMRect }): void
}>()
